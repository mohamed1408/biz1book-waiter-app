...
export default function PickupScreen({ navigation }: RootTabScreenProps<'Pickup'>) {
    const orderstatuses = [
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.getdata(new URL('finddata', url).href, { dbname: "preorders", findQuery: { OrderTypeId: 4 } })
        ...
        <View style={styles.container}>
            <View style={[{ flex: 1, flexDirection: 'row', maxHeight: '8%' }]}>
                ...
                <TouchableOpacity
                    ...
                    <Text style={[{ color: 'white' }]}>New Pickup</Text>
                </TouchableOpacity>
            </View>
            <FlatList
                data={orders}
